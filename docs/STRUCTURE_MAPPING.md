...
- The settings bottom action opens a catalog-owned utility surface. `WorkSurface.svelte` must render operation-boundary rows from `settingsUtilityItems` and only treat `settingsNavigationItems` as shortcuts to existing editor product surfaces.
